Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/989c5f7a47061384d91a00a4cf02c8c0"
{
	Properties
	{
...
#define PROP_EMISSIONSTRENGTH 11.47
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			{
				float3 emission0 = 0;
				float emissionStrength0 = float(11.47);
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.4339623,0,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.4339623,0,0,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(0.4339623,0,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
