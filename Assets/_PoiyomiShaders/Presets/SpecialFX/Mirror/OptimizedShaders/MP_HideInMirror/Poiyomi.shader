Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/5bc9ee9a1e88a4e4eafa886eda898605"
{
	Properties
	{
...
#define PROP_ENABLEMIRROROPTIONS 1
#define PROP_MIRROR 2
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyMirror(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(2) != 0)
				{
					if (float(2) == 1 && inMirror)	return;
					if (float(2) == 1 && !inMirror)	discard;
					if (float(2) == 2 && inMirror)	discard;
					if (float(2) == 2 && !inMirror)	return;
				}
				#if(defined(FORWARD_BASE_PASS) || defined(FORWARD_ADD_PASS))
				#if defined(PROP_MIRRORTEXTURE) || !defined(OPTIMIZER_ENABLED)
				if(inMirror)
				{
					poiFragData.baseColor = POI2D_SAMPLER_PAN(_MirrorTexture, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0));
				}
				#endif
				#endif
			}
